...
const changeHello = () => {
  sayHelloButton.className = "changeHello";
};
...
const ogHello = () => {
  sayHelloButton.className = "";
};

sayHelloButton.addEventListener("mouseout", ogHello);
...
const ohMy = () => {
  ...
  axios.get("http://localhost:3000/animals").then((res) => {
    for (i = 0; i < res.data.length; i++) {
      let animalTag = document.createElement("p");
      animalTag.textContent = res.data[i];
      document.querySelector("body").append(animalTag);
    }
  });
};
...
const repeatMyParam = () => {
  ...
  axios.get("http://localhost:3000/repeat/someparam").then((res) => {
    const repeatBtn = document.querySelector("#repeat-text");
...
const queryFunc = () => {
  axios
    .get("http://localhost:3000/query-test/?question=MyQuestion")
    .then((res) => {
...
/*
    ...
    {message: 'You sent more than 1 query!', queries: {…}}
    message: "You sent more than 1 query!"
    queries: {question: '', two: ''}
    [[Prototype]]: Object
...
const createFood = (event) => {
  ...
  axios.post("http://localhost:3000/food", body).then((res) => {
    ...
    foodPara.textContent = res.data;
    document.querySelector("body").append(foodPara);
  });
};

document.getElementById("foodForm").addEventListener("submit", createFood);
